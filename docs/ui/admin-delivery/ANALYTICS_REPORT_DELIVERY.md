...
- AI 报表中心当前承接真实 AI 周报、月报摘要草稿、异常解释和导出送达建议。
- 主区应只保留摘要草稿、异常列表和送达动作，报表定位、导出边界和操作说明迁移到后置上下文和帮助页。
- 保持 AI 报表中心只做草稿和人工确认出口，不直接自动发布正式经营报表；AI 结果不可用时显式显示 unavailable，不再回退 mock 报表。

## Data Source

- Route type: client report summary page
- Primary sources: `/api/ai/ops-report`, `/api/ai/dashboard-insights`, `/api/dashboard/overview`
- Downstream dependencies: shared PageHeader, StatCard, DataCard and report period state

## UI States

- Loading state: 周报与月报切换会触发真实报表重取，需显示生成反馈。
- Empty state: 若当前周期没有可纳入的真实摘要内容，应显式提示 live empty，而不是回退本地草稿。
- Error state: 报表摘要、风险解释与 dashboard 摘要口径不一致时应显式暴露，并保留帮助入口。
- Mobile impact: 摘要区、导出建议区、周期按钮组和帮助卡需要验证窄屏折行表现。

## Health Signals

- Healthy signal: 报表中心在周报、月报切换下稳定生成真实摘要草稿，并保持主区聚焦结果、说明后置。
- Failure signal: 周期切换失效、异常解释与 dashboard 分叉、导出建议脱离当前摘要上下文，或接口失败时回退 mock 报表。
...
- If regressions appear, fallback is the previous analytics report layout, not a restored local AI report source.
