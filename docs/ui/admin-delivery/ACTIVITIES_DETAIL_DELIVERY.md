...
- 返回列表、活动 KPI 卡片和详情字段仍保留，但数据来源已切到共享 workflow。
...
- Mobile impact: 详情页头、状态卡、统计卡和发布 CTA 在窄屏下仍需保持主操作可见。

## Health Signals

- Healthy signal: 从列表或新建回流进入后，标题、状态卡、发布动作和详情字段保持一致。
...
- Manual path: 验证从活动列表进入详情、待发布活动发布、返回列表、统计卡和详情字段展示
