...
- Error state: 必填项或严重事故缺少下一步动作时在表单顶部显式提示。
...
- Revert this delivery note together with incidents new route and shared operations workflow changes.
- If regressions appear, fallback is to remove incidents new route and restore the previous static list-only behavior.