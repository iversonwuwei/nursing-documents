...
- 当前交付单元通过真实 `/api/ai/rules` 与 `/api/ai/audit-logs` 展示治理状态；接口失败时显式暴露 unavailable，不回退本地规则 mock。

## Data Source

- Route type: client page with query-param context and local rule toggle state
- Primary sources: `/api/ai/rules`, `/api/ai/audit-logs`, ai-context helpers
- Downstream link: logs audit route with appended tracking context

## UI States

- Loading state: 首屏拉取真实规则与相关审计日志时显示可见反馈。
- Empty state: 当前若无相关规则或相关日志，应保持局部空态而不破坏治理框架；帮助入口仍需可达。
- Error state: rule context 与 related logs/rules mapping 错位，或真实规则接口失败时需显式暴露。
- Mobile impact: 规则卡片信息量大，后续变更需验证窄屏下主区规则列表优先级与回滚说明可读性。

## Health Signals

- Healthy signal: tracking context、真实规则启停和转到日志审计的路径保持一致，后置治理说明不遮挡主区操作。
- Failure signal: 规则映射与日志建议错位、治理边界缺乏可回滚说明，或接口失败时回退 mock 规则。
