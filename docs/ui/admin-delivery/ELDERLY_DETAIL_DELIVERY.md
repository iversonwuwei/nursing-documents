...
- 长者详情页承担对象主档、健康摘要、家属摘要草稿和管理动作建议的合流入口。
- 当前交付单元先固定说明、健康信号和回滚要求，不修改页面行为。
- 保持现有 AI 摘要、家属摘要草稿和详情编辑入口不变。

## Data Source

- Route type: client detail route with params-based local data lookup
- Primary sources: local elder detail mock data, admin AI helpers, family AI profile helpers
...
- Empty state: 未命中对象时当前回退默认对象，后续若接真实数据需显式无对象状态。
- Error state: AI 摘要与详情数据不一致时应局部可见，不能全页静默。
- Mobile impact: 详情多卡片双栏布局，后续变更要确认窄屏堆叠顺序与 CTA 可达性。

## Health Signals

- Healthy signal: 对象主档、AI 摘要和管理动作卡片在同一对象上下文下保持一致。
- Failure signal: back link、AI 上下文链接或详情对象映射错位。
...
- Manual path: 验证 elderly list -> detail -> AI context link 的对象一致性
